...
g1=np.array([(-math.log(2*math.pi) - (math.log(np.linalg.det(ocov)))/2 - (np.matmul(np.transpose(np.array([i,j])-mf),np.matmul(np.linalg.inv(ocov),(np.array([i,j])-mf))))/2 + math.log(prf))  for j in y for i in x ])
g2=np.array([(-math.log(2*math.pi) - (math.log(np.linalg.det(ocov)))/2 - (np.matmul(np.transpose(np.array([i,j])-ms),np.matmul(np.linalg.inv(ocov),(np.array([i,j])-ms))))/2 + math.log(prs))  for j in y for i in x ])
fn=(g1-g2)
F=fn.reshape(300,300)
G=g1.reshape(300,300)
H=g2.reshape(300,300)
for i in range (0, 90):
	if(test_label[i]==1):
...
plt.ylabel("X2")
print(ocov)
plt.title("Iso-Probability and Discriminant Function (A) train data")
plt.figure(2)
plt.contour(x,y,G,levels=[-8,-7,-6,-5,-4,-3.5,-3,-2])
plt.contour(x,y,H,levels=[-8,-7,-6,-5,-4,-3.5,-3,-2])
plt.contour(x,y,F,levels=[0])
...
plt.xlabel("X1")
plt.ylabel("X2")
plt.title("Iso-Probability and Discriminant Function (A) test data")
plt.show()
