...
for i in range (0,90):
	pdff=-math.log(2*math.pi) - (math.log(df))/2 - (np.matmul(np.transpose(test_data[i]-mf),np.matmul(ins,(test_data[i]-mf))))/2 + math.log(prf)
	pdfs=-math.log(2*math.pi) - (math.log(df))/2 - (np.matmul(np.transpose(test_data[i]-ms),np.matmul(ins,(test_data[i]-ms))))/2 + math.log(prs)
	if(pdff>=pdfs):
		l.append(1)
	else:
		l.append(0)
...
for i in range (0,90):
	...
	if(test_label[i]==0):
		lcs=lcs+1
print "\n"
print "When the Covariance is calculated for each class and weighted mean is calculated"
